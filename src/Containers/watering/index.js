...
function WateringContainer(props) {
    const { device } = props;
    const [programs, setPrograms] = useState(JSON.parse(localStorage.getItem(`programs_${device.id}`)) || []);
    ...
    const saveActiveProgram = () => {
        ...
        window.ble.write(device.id, SERVICE_UUID, CHARACTERISTIC_PROGRAM, `${program.startTime.replace(':', '')};${program.duration * 60};${program.interval};${program.smart?'1':'0'}`, () => { console.log('write')}, () => {});
    ...
    const onSubmit = (event) => {
        ...
        if (addProgram) {
            ...
            localStorage.setItem(`programs_${device.id}`, JSON.stringify([...programs, { ...inputs }]));
        } else {
            const currentPrograms = [...programs];
            currentPrograms[selectedProgram - 1] = { ...inputs };
            setPrograms(currentPrograms);
            console.log(currentPrograms);
            localStorage.setItem(`programs_${device.id}`, JSON.stringify(currentPrograms));
        ...
        <Fragment>
            {!(addProgram || selectedProgram > 0) && <div className="uk-section uk-section-small">
                <div className="uk-container">
                    {programs.length > 0 && activeProgram === 0 && <div uk-alert="true">
                        Es ist noch kein Programm aktiv.
                    </div>}
                    {activeProgramChanged && <div className="uk-margin">
                        <div uk-alert="true">
                            <p>Aktives Programm wurde verändert. Die Änderung muss an das Smartgarden Gerät übermittelt werden.</p>
                            <button className="uk-button uk-button-default uk-width-1-1" type="button" onClick={saveActiveProgram}>Änderung Speichern</button>
                        </div>
                        
                    </div>}
                    {programs.length === 0 && <ProgramNotFound />}
                    {programs.length > 0 && programs.map((program, i) => <ProgramItem key={i + 1} program={program} label={`Programm ${i + 1}`} onSelectProgramm={() => { onSelectProgramm(i+1)}} active={i + 1 === activeProgram} onSetProgramActive={(event) => { onSetProgramActive(event, i+1)}} />)}

                    <button className="uk-button uk-button-primary uk-width-1-1 uk-padding-remove-horizontal" type="button" onClick={addNewProgram}><span uk-icon="icon: plus"></span> Hinzufügen</button>
