...
function HomeContainer(props) {
    ...
    const handleValveClick = () => {
        if(watering === "1") {
            window.ble.write(device.id, SERVICE_UUID, CHARACTERISTIC_VALVE, stringToBytes("0"), (data) => {
                setWatering("0");
            });
        } else {
            window.ble.write(device.id, SERVICE_UUID, CHARACTERISTIC_VALVE, stringToBytes("1"), (data) => {
                setWatering("1");
            });
        }
        ...
        if(connected && device !== null) {
            ...
            const readData = () => {
                window.ble.read(device.id, SERVICE_UUID, CHARACTERISTIC_TEMP, (data) => {
                    ...
                    setTemperatures((temperatures) => [...temperatures, bytesToString(data)]);
                }, () => { });
    
                window.ble.read(device.id, SERVICE_UUID, CHARACTERISTIC_TEMP_GROUND, (data) => {
                    setTemperatureGround(bytesToString(data));
                    setTemperaturesGround((temperaturesGround) => [...temperaturesGround, bytesToString(data)]);
                }, () => { });
    ...
    return (
        <>
                { connected && !showDiagram && <Dashboard temperature={temperature} temperatureGround={temperatureGround} moisture={moisture} watering={watering} setShowDiagram={setShowDiagram} handleValveClick={handleValveClick} /> }
                { connected && showDiagram && <Diagram temperatures={temperatures} temperaturesGround={temperaturesGround} moistures={moistures} onCancel={() => { setShowDiagram(false); }} /> }
        </>
    );
}
